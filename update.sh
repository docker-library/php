...
for version in "${versions[@]}"; do
	...
	for suite in stretch jessie alpine{3.9,3.8}; do
		...
		for variant in cli apache fpm zts; do
			...
			if [ "${suite/alpine*/alpine}" = 'alpine' ] && [ "$variant" = 'apache' ]; then
				dockerFileVariant="alpine-$variant"
			fi

			echo "Generating $version/$suite/$variant/Dockerfile from $baseDockerfile + $dockerFileVariant-Dockerfile-block-*"

			gawk -i inplace -v dockerFileVariant="$dockerFileVariant" '
				$1 == "##</autogenerated>##" { ia = 0 }
				!ia { print }
				$1 == "##<autogenerated>##" { ia = 1; ab++; ac = 0; if (system("test -f " dockerFileVariant "-Dockerfile-block-" ab) != 0) { ia = 0 } }
				ia { ac++ }
				ia && ac == 1 { system("cat " dockerFileVariant "-Dockerfile-block-" ab) }
